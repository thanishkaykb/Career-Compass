      ...
      {showApply && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setShowApply(false)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={apply} className="glass rounded-2xl p-6 w-full max-w-lg space-y-3 max-h-[90vh] overflow-y-auto">
            ...
            <textarea value={form.resume_text} onChange={(e) => setForm({ ...form, resume_text: e.target.value })} rows={6} placeholder="Paste your resume" className="w-full rounded-xl bg-surface-2 px-3 py-2.5 text-sm border border-border" />
            <div className="flex justify-end gap-2">
              ...
              <button disabled={submitting} className="rounded-xl bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60">{submitting ? "Submitting…" : "Submit application"}</button>
            </div>
          </form>
        </div>
      )}
